...
import type { DatabaseAdapter, DatabaseDriver, ExecutionResult, QueryOptions, TransactionOptions } from "./adapter";
...
class PgAdapter implements DatabaseAdapter {
  ...
  async transaction<T>(callback: (adapter: DatabaseAdapter) => Promise<T>, options: TransactionOptions = {}): Promise<T> {
    if (this.client) {
      const savepoint = `askr_${Math.random().toString(36).slice(2)}`;
      await this.client.query(`SAVEPOINT ${savepoint}`);
      try { const value = await callback(this); await this.client.query(`RELEASE SAVEPOINT ${savepoint}`); return value; }
      catch (error) { await this.client.query(`ROLLBACK TO SAVEPOINT ${savepoint}`); throw error; }
    }
    if (!this.pool) throw new Error("PostgreSQL transactions require a pool.");
    const client = await this.pool.connect();
    try {
      const clauses = [options.isolation && `ISOLATION LEVEL ${options.isolation.toUpperCase()}`, options.readOnly && "READ ONLY"].filter(Boolean).join(" ");
      await client.query(`BEGIN${clauses ? ` ${clauses}` : ""}`);
      const value = await callback(new PgAdapter(client as never, this.identity, undefined, client));
  ...
  async session<T>(callback: (adapter: DatabaseAdapter) => Promise<T>): Promise<T> {
    ...
    const client = await this.pool.connect();
    try { return await callback(new PgAdapter(client as never, this.identity, undefined, client)); }
    finally { client.release(); }
  }
  ...
  async migrationLock<T>(callback: (adapter: DatabaseAdapter) => Promise<T>): Promise<T> {
    return this.session(async (session) => {
      ...
      try { return await callback(session); }
        ...
        await session.execute({ text: `SELECT pg_advisory_unlock(${MIGRATION_LOCK_KEY})`, values: [] }).catch(() => undefined);
      }
    });
  }
...
async function createPool(url: string, options?: Omit<PoolConfig, "connectionString">): Promise<PoolType> {
  let module: typeof import("pg");
  try { module = await import("pg"); }
  catch (error) { throw new Error("PostgreSQL support requires optional peers `pg` and `pg-query-stream`.", { cause: error }); }
  try { await import("pg-query-stream"); }
  catch (error) { throw new Error("PostgreSQL support requires optional peer `pg-query-stream`.", { cause: error }); }
  return new module.Pool({ ...options, connectionString: url });
}

async function pgTooling(url: string, options?: Omit<PoolConfig, "connectionString">): Promise<DatabaseToolingAdapter> {
  const pool = await createPool(url, options);
  return {
    identity: url,
    async reset() { throw new Error("PostgreSQL shadow reset must be performed by migration tooling."); },
    async execute(sql) { await pool.query(sql); },
    async introspect() { return (await pool.query("SELECT current_database() AS database, current_schema() AS schema")).rows; },
...
export function postgres(options: PostgresOptions = {}): DatabaseDriver {
  return {
    dialect: "postgres",
    get targetIdentity() { return lazy(options.url, "DATABASE_URL"); },
    get shadowIdentity() { return lazy(options.shadowUrl, "DATABASE_SHADOW_URL"); },
    async open() {
    ...
    async shadow() {
      ...
      if (target === shadow) throw new Error("PostgreSQL target and shadow database identities must differ.");
