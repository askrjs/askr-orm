...
interface QueueContext { readonly adapter: SqliteAdapter }
...
class SqliteAdapter implements DatabaseAdapter {
  ...
  constructor(private readonly database: DatabaseSync, identity: string, queue = new SqliteQueue()) {
  ...
  async *stream<Row>(query: SqlQuery, options: QueryOptions = {}): AsyncIterable<Row> {
    const release = context.getStore()?.adapter === this
      ? () => undefined
  ...
  transaction<T>(callback: (adapter: DatabaseAdapter) => Promise<T>, _options?: TransactionOptions): Promise<T> {
    if (context.getStore()?.adapter === this) return this.nested(callback);
    return this.queue.run(() => context.run({ adapter: this }, async () => {
  ...
  async close(): Promise<void> {
    await this.queue.run(async () => { this.closed = true; this.database.close(); });
  }
}
...
function tooling(filename: string): DatabaseToolingAdapter {
  ...
  return {
    ...
    async execute(sql) { database.exec(sql.replaceAll('"public".', "")); },
    async introspect() {
      const objects = database.prepare("SELECT type, name, sql FROM sqlite_schema WHERE name NOT LIKE 'sqlite_%' ORDER BY name").all() as Array<{ type: string; name: string; sql: string | null }>;
      const tables = objects.filter((entry) => entry.type === "table").map((entry) => {
        ...
        for (const index of indexes.filter((value) => value.unique && value.origin === "u")) {
          ...
          const indexed = database.prepare(`PRAGMA index_info("${safeIndex}")`).all() as Array<{ name: string }>;
      ...
      const views = objects.filter((entry) => entry.type === "view").map((entry) => ({
        kind: "view" as const,
        schema: "public",
        name: entry.name,
    ...
    async describe(sql, parameterNames) {
      ...
      return {
        ...
        columns: statement.columns().map((column) => ({ name: column.name, dataType: column.type ?? "unknown", nullable: true })),
      };
    },
...
export function sqlite(options: SqliteOptions = {}): DatabaseDriver {
  ...
  return {
    ...
    async open() { return new SqliteAdapter(new DatabaseSync(filename), identity); },
    async shadow() { return tooling(":memory:"); },
  };
}
